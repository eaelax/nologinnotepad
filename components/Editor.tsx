...
const EditorComponent: React.FC<EditorProps> = ({
  ...
  // Sync editor content on initial mount and when active note changes
  useEffect(() => {
    if (editableRef.current) {
      const formattedHtml = convertPlainTextToHtml(note.content || '');
      if (activeNoteIdRef.current !== note.id || editableRef.current.innerHTML !== formattedHtml) {
        editableRef.current.innerHTML = formattedHtml;
      }
  ...
  // Handle external updates to note content (e.g. file import or restore) when editor is not actively focused
  useEffect(() => {
    if (editableRef.current && document.activeElement !== editableRef.current) {
      const formattedHtml = convertPlainTextToHtml(note.content || '');
      if (editableRef.current.innerHTML !== formattedHtml) {
        editableRef.current.innerHTML = formattedHtml;
        localHtmlRef.current = note.content || '';
        setCurrentText(getPlainTextWithNewlines(note.content || ''));
      }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(focusEditor, 80);
    return () => {
  ...
  const handleInput = useCallback(() => {
    ...
    // Immediately notify onContentChange so parent updates state and saves to localStorage
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    // Ctrl/Cmd + S: Trigger file download (prevent default browser save dialog) and save immediately
    ...
    if (hasMod && e.shiftKey && e.key.toLowerCase() === 'c') {
      e.preventDefault();
      const plainText = stripHtmlToPlainText(editableRef.current?.innerHTML || note.content || '');
